...
{
    public async Task<IReadOnlyList<InsurancePolicyDto>> ListAsync(Guid patientId, CancellationToken cancellationToken)
    {
        await RequirePatientAsync(patientId, cancellationToken);
        var rows = await db.PatientInsurancePolicies.AsNoTracking().Include(p => p.Authorizations)
            .Where(p => p.PatientId == patientId && !p.IsArchived)
