...
public static class InsurancePolicyEndpoints
{
    public static void MapInsurancePolicyEndpoints(this IEndpointRouteBuilder app)
    {
        ...
        read.MapGet("/",async(Guid patientId,IInsurancePolicyService service,CancellationToken ct)=>await Execute(()=>service.ListAsync(patientId,ct)));
        ...
        write.MapPost("/",async(Guid patientId,UpsertInsurancePolicyRequest request,IInsurancePolicyService service,CancellationToken ct)=>await Execute(()=>service.UpsertPolicyAsync(patientId,null,request,ct),true));
        ...
        write.MapPost("/{policyId:guid}/authorizations",async(Guid patientId,Guid policyId,UpsertInsuranceAuthorizationRequest request,IInsurancePolicyService service,CancellationToken ct)=>await Execute(()=>service.UpsertAuthorizationAsync(patientId,policyId,null,request,ct),true));
