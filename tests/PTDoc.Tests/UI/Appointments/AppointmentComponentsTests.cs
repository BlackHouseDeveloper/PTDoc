...
public sealed class AppointmentComponentsTests : TestContext
{
    ...
    [Fact]
    public void AppointmentDetailModal_ScheduledAppointment_DisablesStartVisitWithReason()
    {
        JSInterop.Mode = JSRuntimeMode.Loose;
        ...
        Assert.Contains("Check in the patient before starting the visit.", reason.TextContent, StringComparison.Ordinal);

        var startVisitButtons = cut.FindAll("button").Where(button => button.TextContent.Contains("Start Visit", StringComparison.Ordinal)).ToList();
        Assert.NotEmpty(startVisitButtons);
        Assert.All(startVisitButtons, button =>
        {
