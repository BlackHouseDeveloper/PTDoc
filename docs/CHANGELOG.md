...
- **`src/PTDoc.Application/Data/WorkspaceReferenceCatalog.json`**, **`src/PTDoc.Api/Notes/NoteWorkspaceV2Endpoints.cs`**, **`src/PTDoc.Infrastructure/Notes/Workspace/WorkspaceReferenceCatalogAssetMapper.cs`**, **`src/PTDoc.Infrastructure/Notes/Workspace/WorkspaceReferenceCatalogService.cs`**, **`tests/PTDoc.Tests/Integration/NoteWorkspaceCatalogEndpointIntegrationTests.cs`**, **`tests/PTDoc.Tests/Notes/Workspace/WorkspaceReferenceCatalogAssetMapperTests.cs`**, **`tests/PTDoc.Tests/Notes/Workspace/WorkspaceReferenceCatalogServiceTests.cs`** — Followed up on PR review hardening by pretty-printing the embedded workspace catalog asset for stable diff review, rejecting numeric or undefined `BodyPart` values during asset mapping, deep-cloning catalog availability/provenance across shared template fan-out and service reads, and returning a 400 from the body-region catalog endpoint when route model binding produces an undefined enum value. Reason: the initial migration worked functionally, but the review surfaced input-validation and isolation edges worth tightening before merge.
- **`src/PTDoc.Infrastructure/Notes/Workspace/WorkspaceCatalogCloneHelpers.cs`**, **`src/PTDoc.Infrastructure/Notes/Workspace/WorkspaceReferenceCatalogAssetMapper.cs`**, **`src/PTDoc.Infrastructure/Notes/Workspace/WorkspaceReferenceCatalogService.cs`**, **`tests/PTDoc.Tests/Notes/Workspace/WorkspaceReferenceCatalogAssetMapperTests.cs`**, **`tests/PTDoc.Tests/Notes/Workspace/WorkspaceReferenceCatalogServiceTests.cs`** — Tightened the final review pass by switching asset body-part parsing to named-enum members only so numeric tokens like `99` are rejected even when they match a defined enum backing value, distinguishing undefined `BodyPart` caller input from true asset-completeness failures inside `WorkspaceReferenceCatalogService`, and centralizing workspace catalog provenance/availability deep-clone helpers to avoid drift between the mapper and service. Reason: the last Copilot review surfaced one remaining numeric-enum loophole and one avoidable clone-helper duplication after the earlier hardening pass.
- **`src/PTDoc.Infrastructure/Notes/Workspace/WorkspaceReferenceCatalogAssetMapper.cs`**, **`tests/PTDoc.Tests/Notes/Workspace/WorkspaceReferenceCatalogAssetMapperTests.cs`** — Tightened shared-section validation so an omitted or half-empty `shared` block in `WorkspaceReferenceCatalog.json` now fails fast during mapping instead of silently deserializing into default all-missing values via property initializers. Reason: the final PR review correctly called out that the contract’s `new()` defaults masked missing shared section payloads unless the mapper validated required shared metadata explicitly.
- **`src/PTDoc.Application/Data/WorkspaceReferenceCatalog.json`**, **`src/PTDoc.Infrastructure/Notes/Workspace/WorkspaceReferenceCatalogAssetMapper.cs`**, **`src/PTDoc.Infrastructure/Notes/Workspace/WorkspaceReferenceCatalogService.cs`**, **`src/PTDoc.UI/Services/NoteWorkspacePayloadMapper.cs`**, **`src/PTDoc.UI/Pages/Patient/NoteWorkspacePage.razor`**, **`tests/PTDoc.Tests/Notes/Workspace/WorkspaceReferenceCatalogAssetMapperTests.cs`**, **`tests/PTDoc.Tests/Notes/Workspace/WorkspaceReferenceCatalogServiceTests.cs`**, **`tests/PTDoc.Tests/Notes/Workspace/NoteWorkspacePayloadMapperTests.cs`**, **`tests/PTDoc.Tests/UI/Pages/NoteWorkspacePageTests.cs`**, **`tests/PTDoc.Tests/UI/Notes/PlanTabTests.cs`** — Completed the follow-up hardening by adding explicit unsupported-region templates for `Thoracic` and sentinel `Other`, making asset completeness across every `BodyPart` enum value a mapper invariant, removing the service-manufactured missing-catalog fallback, normalizing load-time selected body part resolution across Subjective and Objective so existing notes keep Objective catalog continuity, guarding the resolver against empty structured collections defaulting to the enum zero value, and expanding focused regressions for lumbar coverage, unsupported-region outcome fallback, mapper precedence conflicts, page-level Objective hydration, and the stable Plan-tab catalog seam with explicit element-presence assertions instead of broad markup substring matches. Reason: the branch still had one service-coded unsupported-region path and one live Objective-tab continuity regression after the initial asset migration landed.
...
- **`src/PTDoc.Infrastructure/Notes/Workspace/NoteWorkspaceV2Service.cs`** — Removed the redundant workspace-local legacy payload translator so workspace load now resolves only through explicit schema-v2 deserialization, the shared `NoteWriteService.NormalizeContentJson(...)` path, and empty-payload fallback for unrecognized JSON. Outcome-measure overlay now only replaces payload values when persisted outcome rows actually exist. Reason: Branch 5’s remaining workspace compatibility island was duplicating legacy note migration logic that the shared canonicalizer already owns.
- **`src/PTDoc.Api/Notes/NoteEndpoints.cs`**, **`src/PTDoc.Infrastructure/Data/ApplicationDbContext.cs`**, **`src/PTDoc.Infrastructure.Migrations.Sqlite/Migrations/20260417160000_RetireLegacyAddendumStore.cs`**, **`src/PTDoc.Infrastructure.Migrations.Postgres/Migrations/20260417160000_RetireLegacyAddendumStore.cs`**, **`src/PTDoc.Infrastructure.Migrations.SqlServer/Migrations/20260417160000_RetireLegacyAddendumStore.cs`**, **`src/PTDoc.Core/Models/Addendum.cs`**, **`tests/PTDoc.Tests/Tenancy/TenantIsolationTests.cs`**, **`tests/PTDoc.Tests/Integration/DatabaseProviderSmokeTests.cs`**, **`tests/PTDoc.Tests/Integration/EndToEndWorkflowTests.cs`** — Retired the legacy `Addendums` store from the runtime model, added provider-specific migrations that backfill legacy table rows into linked `ClinicalNote` addendums while preserving the original IDs, and simplified note-detail addendum reads to use only linked `ClinicalNote` rows. Linked addenda now infer `ContentFormat` from the stored payload so JSON-string addenda return plain text while structured addenda remain JSON. Reason: the legacy table was the last live non-canonical addendum path in Branch 5 even though addendum creation had already moved to linked `ClinicalNote` records.
- **`src/PTDoc.Infrastructure/Pdf/ClinicalDocumentHierarchyBuilder.cs`** — Replaced remaining export placeholder scaffolding with current canonical workspace data where it exists, especially treatment rows from `Objective.ExerciseRows` and `Plan.GeneralInterventions`, treatment comments from plan fields, and precautions from `Plan.FollowUpInstructions`. Unsupported TODO-only sections such as contraindications and daily/discharge carry-forward placeholders are now omitted instead of rendering empty placeholder structures. Reason: Branch 5 closeout requires export output to stop depending on internal TODO scaffolding once the canonical note contract is stable.

### Changed - Branch 5 note write response canonicalization

#### Generic note create/update responses now return canonical workspace-v2 content
- **`src/PTDoc.Infrastructure/Services/NoteWriteService.cs`**, **`tests/PTDoc.Tests/Integration/EndToEndWorkflowTests.cs`** — Updated the internal note-write response mapper so successful create/update envelopes normalize recognized legacy `ContentJson` before returning `NoteResponse`, and tightened the existing end-to-end note create/update tests to assert the response payload is canonical workspace v2 in addition to the stored database row. Reason: earlier Branch 5 slices had canonicalized generic note persistence and read endpoints, but `POST /api/v1/notes` and `PUT /api/v1/notes/{id}` were still echoing raw stored note JSON through the service response envelope.

### Changed - Branch 5 signature-eligibility enforcement alignment

#### Signature-eligibility evaluation now mirrors the canonical pre-sign contract instead of only checking signed state
- **`src/PTDoc.Infrastructure/Compliance/RulesEngine.cs`**, **`tests/PTDoc.Tests/Compliance/RulesEngineTests.cs`** — Updated `ValidateSignatureEligibilityAsync` to run recognized legacy note content through the shared canonicalizer, reject notes that still lack canonical diagnosis codes, and surface blocking clinical/compliance violations from `IClinicalRulesEngine` as hard-stop signature-ineligibility results, with focused tests covering both missing-diagnosis and blocking-clinical-violation cases. Reason: after earlier Branch 5 slices aligned the actual sign endpoint with canonical workspace-v2 validation, the public `POST /api/v1/compliance/evaluate/signature-eligible/{noteId}` evaluator could still return “eligible” for notes the real sign flow would reject.
- **`src/PTDoc.Infrastructure/Compliance/RulesEngine.cs`**, **`tests/PTDoc.Tests/Compliance/RulesEngineTests.cs`** — Aligned addendum behavior with the actual sign path by exempting draft addenda from the new canonical diagnosis and blocking-clinical checks in `ValidateSignatureEligibilityAsync`, and added focused coverage for addendum eligibility. Reason: the sign flow already allows addenda to be signed independently without the parent note completeness contract, so the public eligibility evaluator must not introduce a stricter rule than the sign endpoint itself.

### Changed - Branch 5 AI acceptance legacy-workspace canonicalization

#### AI suggestion acceptance now reuses the shared canonicalizer for legacy workspace-shaped note content
- **`src/PTDoc.Api/Notes/NoteEndpoints.cs`**, **`tests/PTDoc.Tests/Notes/SoapGammaEnforcementTests.cs`**, **`tests/PTDoc.Tests/Integration/EndToEndWorkflowTests.cs`** — Updated the `accept-ai-suggestion` workspace merge helper to load note content through `NoteWriteService.NormalizeContentJson(...)` when the stored draft is a recognized legacy workspace-shaped payload, and added focused unit plus HTTP coverage proving structured diagnosis codes, goals, and CPT selections survive the AI merge while the note is backfilled to canonical workspace v2. Reason: earlier Branch 5 slices had already canonicalized generic reads, writes, signing, sync, exports, and helper services, but accepting AI text into a draft note could still bypass the shared canonicalizer unless the stored payload was already workspace v2 or a flat SOAP-section object.

### Changed - Branch 5 clinical validation canonicalization

#### Clinical note validation now evaluates the shared canonical note payload for recognized legacy workspace content
- **`src/PTDoc.Infrastructure/Compliance/ClinicalRulesEngine.cs`**, **`src/PTDoc.Infrastructure/Services/NoteWriteService.cs`**, **`tests/PTDoc.Tests/Compliance/ClinicalRulesEngineTests.cs`** — Updated the clinical rules engine to run note `ContentJson` through `NoteWriteService.NormalizeContentJson(...)` before parsing and structured validation, and extended the legacy flat-JSON migration path to preserve root-level `planOfCare`, `certificationPeriod`, and `functionalLimitations` fields when they are promoted into workspace v2. Reason: after earlier Branch 5 slices canonicalized note save, read, signing, sync, exports, and helper surfaces, direct compliance validation could still judge recognized legacy workspace notes against their raw stored JSON instead of the canonical workspace-v2 shape the rest of the system now uses.

### Changed - Branch 5 daily-note helper legacy-workspace canonicalization

#### Daily-note helper reads and JSON requests now reuse the shared note canonicalizer for legacy workspace-shaped content
- **`src/PTDoc.Infrastructure/Services/DailyNoteService.cs`**, **`tests/PTDoc.Tests/Notes/DailyNoteServiceTests.cs`** — Updated the daily-note compatibility service to run stored note bodies and JSON helper-request payloads through `NoteWriteService.NormalizeContentJson(...)` before mapping them into `DailyNoteContentDto`, with focused tests covering both a stored legacy workspace-shaped daily note and a legacy workspace-shaped helper JSON request. Reason: after earlier Branch 5 slices canonicalized generic note APIs, sync, exports, signing, and workspace loads, the dedicated daily-note helper service could still return an empty DTO or ignore structured legacy workspace sections unless the payload already carried `schemaVersion: 2`.

### Changed - Branch 5 shared legacy-workspace canonicalization

#### Shared note canonicalization now migrates legacy workspace-shaped note payloads and signing persists the canonical result
- **`src/PTDoc.Infrastructure/Services/NoteWriteService.cs`**, **`src/PTDoc.Infrastructure/Compliance/SignatureService.cs`**, **`tests/PTDoc.Tests/Compliance/SignatureServiceTests.cs`** — Expanded the shared `NoteWriteService.NormalizeContentJson(...)` migration path to recognize broader legacy workspace-shaped note JSON (`subjective`/`objective`/`assessment`/`plan` sections, diagnosis codes, goals, CPT selections, and outcome aliases), and updated the signing path to normalize tracked draft note content before pre-sign validation and hashing so successful signatures persist canonical workspace-v2 note bodies instead of finalizing recognized legacy workspace payloads unchanged. Reason: after earlier Branch 5 slices moved most read/write/sync/export flows onto the shared canonicalizer, older workspace-shaped draft notes could still reach signature time as non-canonical JSON even though the platform already understood how to represent them as workspace v2.

### Changed - Branch 5 shared dry-needling canonicalization

#### Shared note canonicalization now recognizes legacy dry-needling workspace payloads
- **`src/PTDoc.Infrastructure/Services/NoteWriteService.cs`**, **`tests/PTDoc.Tests/Integration/EndToEndWorkflowTests.cs`** — Extended the shared `NoteWriteService.NormalizeContentJson(...)` legacy migration path to recognize old dry-needling workspace JSON (`workspaceNoteType: "Dry Needling Note"` plus `dryNeedling` fields) and project it into canonical workspace-v2 `dryNeedling` content, with focused generic note-read HTTP coverage proving the shared normalizer now handles that payload outside the workspace-specific service. Reason: Branch 5 had already moved generic reads, writes, sync, carry-forward, exports, and addendums onto the shared note canonicalizer, but legacy dry-needling workspace payloads were still only understood by the dedicated workspace translator.

### Changed - Branch 5 workspace load typed-legacy canonicalization

#### Workspace load now reuses the shared note canonicalizer for recognized typed legacy note content
- **`src/PTDoc.Infrastructure/Notes/Workspace/NoteWorkspaceV2Service.cs`**, **`tests/PTDoc.Tests/Notes/Workspace/NoteWorkspaceV2ServiceTests.cs`** — Updated workspace load deserialization to run note content through `NoteWriteService.NormalizeContentJson(...)` before falling back to the dedicated legacy-workspace translator, and added a focused test covering a draft legacy typed `EvaluationContent` payload. Reason: Branch 5 had already canonicalized most note write, read, sync, export, and helper paths, but opening an older typed evaluation/progress/discharge note through the workspace service could still bypass the shared note canonicalizer and load as an effectively empty workspace payload.

### Changed - Branch 5 daily-note save endpoint canonical input support

#### Daily-note save now accepts canonical workspace-v2 content without dropping the legacy request contract
- **`src/PTDoc.Application/DTOs/DailyNoteDtos.cs`**, **`src/PTDoc.Application/Services/IDailyNoteService.cs`**, **`src/PTDoc.Infrastructure/Services/DailyNoteService.cs`**, **`src/PTDoc.Api/Notes/DailyNoteEndpoints.cs`**, **`tests/PTDoc.Tests/Notes/DailyNoteServiceTests.cs`**, **`tests/PTDoc.Tests/Integration/EndToEndWorkflowTests.cs`** — Added an additive JSON-body save request type for `/api/v1/daily-notes/`, a matching service overload that resolves `content` from either canonical workspace-v2 JSON or the legacy `DailyNoteContentDto` shape, and focused service plus HTTP coverage proving canonical daily note content can now be saved through the legacy daily-note endpoint while still persisting canonical workspace-v2 `ContentJson`. Reason: after Branch 5 canonicalized the generic note APIs, helper endpoints, sync, export, addendums, and carry-forward paths, the dedicated daily-note save endpoint was still a live API surface that only accepted the legacy daily-note DTO contract.

### Changed - Branch 5 evaluation carry-forward canonicalization

#### Daily-note evaluation carry-forward now reuses canonical note normalization for recognized legacy evaluation content
- **`src/PTDoc.Infrastructure/Services/DailyNoteService.cs`**, **`tests/PTDoc.Tests/Notes/DailyNoteServiceTests.cs`** — Updated `GetEvalCarryForwardAsync` to run the source evaluation note body through the shared note-content canonicalizer before projecting activities, diagnosis, and plan-of-care details, and added a focused test covering a typed legacy `EvaluationContent` payload. Reason: after Branch 5 canonicalized note storage and most read surfaces, the daily-note evaluation carry-forward helper was still manually parsing legacy evaluation JSON instead of letting the shared note normalizer convert recognized legacy evaluation content into workspace v2 first.

### Changed - Branch 5 daily-note helper canonical input support

#### Daily-note helper endpoints now accept canonical workspace-v2 note payloads without dropping legacy DTO support
- **`src/PTDoc.Application/Services/IDailyNoteService.cs`**, **`src/PTDoc.Infrastructure/Services/DailyNoteService.cs`**, **`src/PTDoc.Api/Notes/DailyNoteEndpoints.cs`**, **`tests/PTDoc.Tests/Notes/DailyNoteServiceTests.cs`**, **`tests/PTDoc.Tests/Integration/EndToEndWorkflowTests.cs`** — Added additive `JsonElement` overloads for the daily-note helper service methods, changed the `generate-assessment` and `check-medical-necessity` HTTP endpoints to accept raw JSON bodies, and resolved those bodies into either canonical workspace-v2 daily note payloads or the legacy `DailyNoteContentDto` shape before running the existing helper logic. Reason: after Branch 5 canonicalized note storage, reads, sync, export, addendums, and workspace editing, these helper endpoints were still one of the last active API surfaces that only accepted the legacy daily-note DTO contract even though the rest of the note system now centers on workspace v2.

### Changed - Branch 5 addendum write canonicalization

#### Structured clinical-note addendums now persist canonical workspace content on create
- **`src/PTDoc.Infrastructure/Compliance/AddendumService.cs`**, **`tests/PTDoc.Tests/Integration/EndToEndWorkflowTests.cs`** — Updated the signed-note addendum creation path to run recognized structured addendum JSON through the shared note-content canonicalizer before storing it in `ClinicalNote.ContentJson`, and added an end-to-end test that signs a note, creates a structured addendum through the HTTP endpoint, and verifies the stored addendum body is workspace v2. Reason: Branch 5 had already canonicalized main note write, read, sync, carry-forward, export, and linked-addendum read paths, but the addendum write path still persisted raw legacy SOAP JSON for structured clinical-note addendums.

### Changed - Branch 5 linked addendum read canonicalization

#### Generic note detail reads now normalize linked clinical-note addendums into canonical workspace content
- **`src/PTDoc.Api/Notes/NoteEndpoints.cs`**, **`tests/PTDoc.Tests/Integration/EndToEndWorkflowTests.cs`** — Updated the linked addendum response mapper used by `GET /api/v1/notes/{id}` to run recognized clinical-note addendum `ContentJson` through the shared note-content canonicalizer before returning it, and extended the existing note-detail integration test to cover a legacy linked addendum alongside the primary legacy note. Reason: Branch 5 had already canonicalized main note detail, batch-read, patient-note list, sync, carry-forward, and export paths, but the detail endpoint still echoed raw legacy JSON for linked clinical-note addendums.

### Changed - Branch 5 progress tracking canonical payload consumption

#### Progress tracking now reads workspace-v2 note content directly instead of mapping through the old UI note payload
- **`src/PTDoc.UI/Services/ProgressTrackingAggregationService.cs`** — Replaced the internal progress-tracking note parser so canonical workspace-v2 notes are read directly for outcome scores and goal statuses, while a narrow case-insensitive legacy fallback still handles older simple note JSON with `objective.outcomeMeasures` and `assessment.goals`. Reason: after Branch 5 canonicalized the active read, write, export, sync, and offline note paths, progress tracking was one of the last consumers still translating canonical note content through the older `NoteWorkspacePayload` model just to read a small subset of data.

### Changed - Branch 5 offline draft canonical workspace persistence

#### MAUI local note drafts now persist the same canonical workspace-v2 payload as the active save path
- **`src/PTDoc.UI/Services/NoteWorkspacePayloadMapper.cs`**, **`src/PTDoc.UI/Services/NoteWorkspaceApiService.cs`**, **`src/PTDoc.Maui/Services/MauiNoteDraftLocalPersistenceService.cs`**, **`tests/PTDoc.Tests/Notes/Workspace/NoteWorkspacePayloadMapperTests.cs`** — Added a shared workspace payload mapper in `PTDoc.UI`, switched the active HTTP save path to reuse it, and changed MAUI local note-draft persistence to store canonical `NoteWorkspaceV2Payload` JSON instead of serializing the older UI payload shape into `LocalClinicalNoteDraft.ContentJson` and sync-queue payloads. Reason: Branch 5 had already canonicalized server write, read, sync, and export paths, but offline clinician authoring could still persist a non-canonical note body locally even though the local draft entity is explicitly documented as canonical workspace content.
- **`src/PTDoc.Infrastructure/LocalData/LocalSyncOrchestrator.cs`**, **`tests/PTDoc.Tests/LocalData/LocalSyncOrchestratorTests.cs`** — Updated local sync pull handling for `ClinicalNote` records to normalize recognized legacy note `ContentJson` into workspace v2 before writing it into `LocalClinicalNoteDraft`, so pulled note snapshots and addenda backfill into the same canonical offline contract even if the incoming sync payload still carries a recognizable legacy note body. Reason: after switching local draft saves to canonical v2, the local pull path was the remaining offline compatibility seam that could still write older note JSON directly into the local draft store.

### Changed - Branch 5 PDF export DTO canonicalization

#### PDF endpoint export DTOs now normalize recognized legacy note content before rendering
- **`src/PTDoc.Api/Pdf/PdfEndpoints.cs`**, **`tests/PTDoc.Tests/Integration/EndToEndWorkflowTests.cs`** — Updated `LoadNoteExportDtoAsync` to reuse the shared note-content canonicalizer so signed notes with recognized legacy SOAP or typed note JSON now reach the PDF renderer as canonical workspace-v2 `ContentJson`, and added an end-to-end export test that captures the renderer input and verifies a legacy signed daily note is normalized before export. Reason: Branch 5 had already canonicalized the interactive read, write, sync, carry-forward, and PDF-composition paths, but the export endpoint was still handing raw stored legacy note JSON to the renderer DTO for older notes.

### Changed - Branch 5 generic note read canonicalization

#### Generic note detail and patient-note read endpoints now return canonical workspace content for recognized legacy notes
- **`src/PTDoc.Api/Notes/NoteEndpoints.cs`**, **`src/PTDoc.Api/Patients/PatientEndpoints.cs`**, **`src/PTDoc.Infrastructure/Services/NoteWriteService.cs`**, **`tests/PTDoc.Tests/Integration/EndToEndWorkflowTests.cs`** — Updated the existing note and patient-note response mappers to normalize recognized legacy `ContentJson` into workspace v2 on read, reusing the shared note-content canonicalizer so `/api/v1/notes/{id}`, `/api/v1/notes/batch-read`, and `/api/v1/patients/{id}/notes` no longer echo older flat SOAP JSON for records that the platform already knows how to canonicalize. Reason: Branch 5 had fixed the active write, carry-forward, AI-acceptance, and sync paths, but the generic read surface could still leak legacy note payloads back into UI flows for older stored records.
- **`src/PTDoc.Infrastructure/Services/NoteWriteService.cs`**, **`tests/PTDoc.Tests/Integration/EndToEndWorkflowTests.cs`** — Updated generic draft note updates to opportunistically canonicalize recognized legacy `ContentJson` even when the request only changes metadata such as `DateOfService`, so an active edit no longer leaves a legacy note body in storage just because the clinician did not rewrite the note text in that request. Reason: the read-side canonicalization fixed what callers receive, but the update path itself still preserved legacy stored payloads when `ContentJson` was omitted from an otherwise successful draft edit.

### Changed - Branch 5 workspace re-evaluation contract preservation

#### Workspace v2 and offline draft flows now preserve `IsReEvaluation`
- **`src/PTDoc.Application/Notes/Workspace/WorkspaceContracts.cs`**, **`src/PTDoc.Infrastructure/Notes/Workspace/NoteWorkspaceV2Service.cs`**, **`src/PTDoc.UI/Services/INoteWorkspaceService.cs`**, **`src/PTDoc.UI/Services/NoteWorkspaceApiService.cs`**, **`src/PTDoc.UI/Components/Notes/Models/SoapNoteVm.cs`**, **`src/PTDoc.UI/Pages/Patient/NoteWorkspacePage.razor`**, **`src/PTDoc.Application/LocalData/Entities/LocalClinicalNoteDraft.cs`**, **`src/PTDoc.Maui/Services/MauiNoteDraftLocalPersistenceService.cs`**, **`src/PTDoc.Infrastructure/LocalData/LocalSyncOrchestrator.cs`**, **`tests/PTDoc.Tests/Notes/Workspace/NoteWorkspaceApiServiceTests.cs`**, **`tests/PTDoc.Tests/Notes/Workspace/NoteWorkspaceV2ServiceTests.cs`**, **`tests/PTDoc.Tests/UI/Pages/NoteWorkspacePageTests.cs`**, **`tests/PTDoc.Tests/LocalData/LocalSyncOrchestratorTests.cs`** — Added `IsReEvaluation` to the workspace v2 load/save contract, persisted it back to `ClinicalNote`, carried it through the active Blazor workspace page state, and extended the MAUI local draft plus sync-queue path so offline saves and pulled note snapshots do not strip the flag. Reason: Branch 5 had already canonicalized note content to workspace v2, but the active workspace contract itself still dropped the re-evaluation bit and forced existing re-eval notes back to plain evaluations on the next save, especially on offline-authoring paths.

### Changed - Branch 5 sync note import canonicalization

#### Offline clinical-note pushes now persist canonical workspace v2 content on the server
- **`src/PTDoc.Infrastructure/Sync/SyncEngine.cs`**, **`tests/PTDoc.Tests/Sync/SyncClientProtocolTests.cs`** — Updated server-side sync import for `ClinicalNote` create/update operations to normalize wrapped `contentJson` payloads through the same workspace-v2 canonicalizer used by the generic note writer before persisting them to `ClinicalNotes`. Recognized legacy note payloads from offline clients now store as canonical workspace v2, while the existing sync protocol and raw/no-op fallback behavior for malformed or unwrapped update payloads remain unchanged. Reason: Branch 5 had already canonicalized the interactive note write paths, and offline sync was the remaining active server write surface that could still persist legacy note JSON directly into note storage.
- **`src/PTDoc.Infrastructure/Sync/SyncEngine.cs`** — Switched sync import `DateTime` parsing from `DateTime.TryParse` to `JsonElement.TryGetDateTime` so ISO 8601 `Z` values such as synced `dateOfService` timestamps preserve round-trip UTC semantics instead of being converted into local time during import. Reason: the first sync-canonicalization cut exposed a real timestamp drift bug in the server import helper, and the sync layer should match the MAUI local parser’s ISO date handling.
- **`src/PTDoc.Infrastructure/Sync/SyncEngine.cs`**, **`tests/PTDoc.Tests/Sync/SyncClientProtocolTests.cs`** — Updated the server `ClinicalNote` delta feed and matching conflict snapshot payload to emit canonical workspace-v2 `ContentJson` for recognized legacy notes instead of replaying raw stored note JSON. Reason: after write-side sync import was canonicalized, the remaining sync read path could still send legacy note content back to MAUI clients and conflict archives, so the delta feed now matches the canonical storage direction.

### Changed - Branch 5 generic carry-forward canonicalization

#### Legacy carry-forward reads now return canonical workspace content when the source note is recognizable
- **`src/PTDoc.Infrastructure/Compliance/CarryForwardService.cs`**, **`src/PTDoc.Infrastructure/Services/NoteWriteService.cs`**, **`src/PTDoc.Application/Compliance/ICarryForwardService.cs`**, **`tests/PTDoc.Tests/Notes/SoapGammaEnforcementTests.cs`** — Updated the generic signed-note carry-forward service to return the same canonical workspace-v2 copy that the generic note writer now produces for recognized legacy note payloads, while leaving unrecognized JSON as-is and never mutating the signed source note in storage. Reason: Branch 5 had already canonicalized the active note write and workspace carry-forward paths, so the remaining `/api/v1/notes/carry-forward` read surface needed to stop re-exposing recognized legacy note JSON on otherwise canonical flows.

### Changed - Branch 5 daily-note compatibility reads

#### Legacy daily-note endpoints now hydrate canonical workspace v2 notes
- **`src/PTDoc.Infrastructure/Services/DailyNoteService.cs`**, **`tests/PTDoc.Tests/Notes/DailyNoteServiceTests.cs`** — Added a narrow compatibility mapper so the legacy daily-note service can read canonical `NoteWorkspaceV2Payload` notes for `GetById`, patient/taxonomy list reads, and evaluation carry-forward instead of deserializing workspace-v2 `ContentJson` into an empty `DailyNoteContentDto`. Reason: Branch 5 has already made workspace v2 the active daily/evaluation note contract, so the remaining `/api/v1/daily-notes` read paths needed to stop silently dropping canonical content when they encounter those saved notes.
- **`src/PTDoc.Infrastructure/Services/DailyNoteService.cs`**, **`tests/PTDoc.Tests/Notes/DailyNoteServiceTests.cs`** — Updated the legacy daily-note save path to persist a canonical workspace-v2 payload in `ContentJson` instead of serializing `DailyNoteContentDto` directly, while keeping the existing request and response DTO surface intact. Reason: Branch 5 should eliminate the last active non-canonical daily-note write path so even the compatibility `/api/v1/daily-notes` workflow stores note content in the same v2 contract as the workspace editor.
- **`src/PTDoc.Infrastructure/Services/DailyNoteService.cs`**, **`tests/PTDoc.Tests/Notes/DailyNoteServiceTests.cs`** — Rehydrated `TreatmentTaxonomySelections` from `NoteTaxonomySelections` when legacy daily-note responses are built from canonical workspace-v2 `ContentJson`, and tightened the new canonical save test to use non-override CPT timing. Reason: moving `ContentJson` to workspace v2 removed the embedded taxonomy list from the legacy DTO projection, so the compatibility surface needed to source those selections from the join table instead of silently returning an empty collection.

### Changed - Branch 5 generic note write canonicalization

#### Generic note CRUD now normalizes recognized legacy note payloads into workspace v2
- **`src/PTDoc.Infrastructure/Services/NoteWriteService.cs`**, **`tests/PTDoc.Tests/Integration/EndToEndWorkflowTests.cs`** — Updated the generic `/api/v1/notes` create/update path to persist canonical workspace-v2 `ContentJson` when callers submit an existing v2 payload, an empty object, plain SOAP-section JSON, or the older typed evaluation/progress/discharge/daily note content DTOs. Reason: Branch 5 still had an active non-canonical note write path outside the workspace-specific endpoint, so the generic note API now normalizes recognized legacy note shapes on save instead of storing them verbatim.
- **`src/PTDoc.Api/Notes/NoteEndpoints.cs`**, **`tests/PTDoc.Tests/Notes/SoapGammaEnforcementTests.cs`**, **`tests/PTDoc.Tests/Integration/EndToEndWorkflowTests.cs`** — Updated `accept-ai-suggestion` to promote empty and legacy SOAP-section note payloads into canonical workspace v2 before applying the accepted AI text, preserving recognized legacy sections instead of rewriting the note as another flat legacy JSON object. Reason: after the generic note writer was canonicalized, AI acceptance was still an active note-mutation path that could keep old draft notes on non-canonical JSON.

### Changed - Branch 5 canonical PDF export alignment

#### Daily and dry-needling exports now read the workspace v2 payload
- **`src/PTDoc.Infrastructure/Pdf/ClinicalDocumentHierarchyBuilder.cs`**, **`tests/PTDoc.Tests/Integration/PdfIntegrationTests.cs`** — Updated PDF export to prefer canonical `NoteWorkspaceV2Payload` for daily notes instead of deserializing the same JSON into `DailyNoteContentDto`, added structured daily-note fallback rendering for subjective/treatment/plan/goals/rehab-potential content, and introduced a dedicated dry-needling PDF document path that renders the canonical `workspace.dryNeedling` payload with its own title and treatment details. Reason: Branch 5 has already made workspace v2 the active daily and dry-needling save contract, so PDF export needed to stop depending on legacy daily-note content shapes for those note types.

### Changed - AGENTS command note sync

#### Added current diagnostics env var and targeted CI category commands
- **`AGENTS.md`** — Added the documented `PTDOC_DEVELOPER_MODE` environment variable for developer diagnostics and the targeted `CoreCi`, `DatabaseProvider`, and `Observability` test filter commands from the current CI docs. Reason: keep the agent instructions aligned with the repo’s current diagnostics toggle and CI-owner command surface without expanding unrelated guidance.

### Changed - AGENTS workflow notes refresh

#### Added current helper-script flags and EF/CI command references
- **`AGENTS.md`** — Added the explicit `dotnet workload install maui` prerequisite for MAUI target work, documented `PTDoc-Foundry.sh --help` and `--verbose`, added the SQLite `dotnet ef dbcontext info` inspection command, and noted that SQL Server/Postgres CI-style migration repro uses `dotnet tool run dotnet-ef` with `EF_PROVIDER`, `Database__ConnectionString`, and a placeholder `Jwt__SigningKey`. Reason: keep the agent instructions aligned with the current repo scripts and CI database workflow without expanding unrelated guidance.
...
- **`src/PTDoc.UI/Services/NoteWorkspaceApiService.cs`**, **`tests/PTDoc.Tests/Notes/Workspace/NoteWorkspaceApiServiceTests.cs`** — Removed the unconditional v1 note-list fetch from workspace note loads. The UI now loads canonical notes from the v2 workspace endpoint first and only consults the legacy note payload for daily notes that still declare `"Dry Needling Note"` and require the old editor path. Reason: Branch 5 should reduce active dependence on legacy note endpoints for standard workspace notes before deleting the remaining compatibility path entirely.
- **`src/PTDoc.Application/Services/IntakeDraftPersistence.cs`**, **`src/PTDoc.Infrastructure/Services/IntakeService.cs`**, **`src/PTDoc.UI/Services/IntakeApiService.cs`**, **`src/PTDoc.Infrastructure/Services/InMemoryIntakeService.cs`**, **`src/PTDoc.Infrastructure/Services/MockIntakeService.cs`**, **`tests/PTDoc.Tests/Intake/IntakeServiceTests.cs`**, **`tests/PTDoc.Tests/Intake/IntakeApiServiceTests.cs`** — Canonical intake structured selections now suppress duplicate legacy supplemental label arrays on active persistence paths. When structured comorbidity, assistive-device, living-situation, or house-layout ids exist, new `ResponseJson` writes clear the mirrored legacy selection sets and service/UI draft hydration normalizes them back to empty. Reason: Branch 5 should stop persisting flattened supplemental fallback data alongside the canonical structured model while still preserving label-array fallback for older drafts that do not yet have structured ids.
- **`src/PTDoc.Infrastructure/Notes/Workspace/NoteWorkspaceV2Service.cs`**, **`tests/PTDoc.Tests/Notes/Workspace/NoteWorkspaceV2ServiceTests.cs`** — Tightened v2 workspace payload detection so note loads only treat `ContentJson` as canonical when the raw JSON explicitly carries `schemaVersion: 2`; legacy note JSON no longer slips through by inheriting the payload class's default schema value during deserialization. Draft notes that successfully translate from legacy workspace JSON are now backfilled to canonical v2 on load, while signed notes, dry-needling compatibility payloads, and invalid legacy JSON are left untouched. Reason: Branch 5 backfill should reduce active server-side legacy translation without mutating legal records or overwriting unknown malformed content.
- **`src/PTDoc.UI/Components/Intake/Models/IntakeSupplementalSelectionCanonicalizer.cs`**, **`src/PTDoc.UI/Components/Intake/Cards/MedicalHistoryCard.razor`**, **`src/PTDoc.UI/Pages/Intake/IntakeWizardPage.razor`**, **`tests/PTDoc.Tests/UI/Intake/StructuredIntakeComponentsTests.cs`** — Added a reusable intake supplemental-selection canonicalizer that promotes legacy comorbidity, assistive-device, living-situation, and house-layout labels into canonical structured ids during draft load and medical-history rendering, and stopped the active intake UI path from mirroring canonical ids back into the legacy state sets. Unmatched legacy labels are still preserved as fallback, but matched legacy selections now upgrade in memory before save and review. Reason: Branch 5 should remove active UI dependence on flattened supplemental label sets without breaking older drafts that have not yet been saved through the canonical path.
- **`src/PTDoc.Infrastructure/Notes/Workspace/NoteWorkspaceV2Service.cs`**, **`tests/PTDoc.Tests/Notes/Workspace/NoteWorkspaceV2ServiceTests.cs`** — Tightened legacy workspace translation again so only recognizable legacy note shapes with nested `subjective`/`objective`/`assessment`/`plan` sections are eligible for translation and draft backfill. Valid but unrelated JSON objects now load as empty v2 shells without mutating stored `ContentJson`. Reason: Branch 5 should stop opportunistically rewriting arbitrary non-v2 note JSON just because it deserializes into the legacy DTO shell.
- **`src/PTDoc.Application/Notes/Workspace/WorkspaceContracts.cs`**, **`src/PTDoc.Infrastructure/Notes/Workspace/NoteWorkspaceV2Service.cs`**, **`src/PTDoc.UI/Services/NoteWorkspaceApiService.cs`**, **`tests/PTDoc.Tests/Notes/Workspace/NoteWorkspaceApiServiceTests.cs`** — Added a `RequiresLegacyCompatibility` flag to the v2 workspace load response and now compute it server-side from the stored note content. The UI only hits the legacy note-list endpoint when that flag is true, so ordinary daily notes no longer pay the extra compatibility fetch while dry-needling notes still do. Reason: Branch 5 should keep shrinking active dependence on legacy note endpoints without removing the one remaining dry-needling compatibility path.
- **`src/PTDoc.UI/Services/NoteWorkspaceApiService.cs`**, **`tests/PTDoc.Tests/Notes/Workspace/NoteWorkspaceApiServiceTests.cs`** — Normalized legacy flat therapeutic-exercise and manual-technique UI lists into structured `exerciseRows` and `generalInterventions` before save, then removed those flat lists as deep merge fallbacks. Compatibility input is still accepted, but the active workspace save path now treats the structured collections as the only durable shape. Reason: Branch 5 should keep eliminating flattened note authoring paths without breaking any older UI state that still submits the legacy lists.
- **`src/PTDoc.UI/Services/NoteWorkspaceApiService.cs`**, **`src/PTDoc.UI/Components/Notes/Workspace/ObjectiveTab.razor`**, **`src/PTDoc.UI/Components/Notes/Models/ObjectiveVm.cs`**, **`tests/PTDoc.Tests/Notes/Workspace/NoteWorkspaceApiServiceTests.cs`**, **`tests/PTDoc.Tests/UI/Notes/StructuredWorkspaceEditorsTests.cs`** — Stopped mirroring structured `exerciseRows` back into the legacy `TherapeuticExercises` list during normal workspace loads and edits. The legacy flat list remains as compatibility-only input for older callers, but the active v2 workspace UI now edits only the structured collection and the mapper no longer recreates duplicate flattened exercise state on load. Reason: Branch 5 should remove the last active flattened exercise-authoring path instead of keeping a shadow copy alive inside the current structured editor flow.
- **`src/PTDoc.Application/Services/IntakeDraftPersistence.cs`**, **`src/PTDoc.Infrastructure/Services/IntakeService.cs`**, **`src/PTDoc.Infrastructure/Services/InMemoryIntakeService.cs`**, **`src/PTDoc.Infrastructure/Services/MockIntakeService.cs`**, **`src/PTDoc.UI/Services/IntakeApiService.cs`**, **`tests/PTDoc.Tests/Intake/IntakeApiServiceTests.cs`**, **`tests/PTDoc.Tests/Intake/IntakeServiceTests.cs`** — Promoted canonical intake consent handling into the shared draft persistence path. Draft `ResponseJson` now backfills a canonical `consentPacket` from legacy intake consent flags before persistence, backend and UI draft hydration both derive convenience booleans from the canonical packet, and the in-memory/mock intake services now preserve cloned consent packets instead of silently dropping them. Legacy `Consents` JSON and patient consent-signing checks now derive from the canonical packet as well. Reason: Branch 5 should make canonical intake consent data the effective source of truth across persistence, hydration, and compatibility layers instead of letting the old mirrored flags drift independently.
- **`src/PTDoc.Application/Services/IntakeDraftPersistence.cs`** — Fixed the shared consent-clone helper so generic draft copies preserve `ConsentPacket = null` instead of materializing an empty packet. Reason: the first Branch 5 consent-persistence cut was wiping legacy convenience flags during seed-state cloning by hydrating them from a synthetic empty packet before canonicalization ran.
- **`src/PTDoc.Application/Services/IntakeDraftPersistence.cs`**, **`src/PTDoc.Infrastructure/Services/IntakeService.cs`**, **`src/PTDoc.UI/Services/IntakeApiService.cs`**, **`tests/PTDoc.Tests/Intake/IntakeApiServiceTests.cs`**, **`tests/PTDoc.Tests/Intake/IntakeServiceTests.cs`** — Removed mirrored legacy consent booleans from persisted intake `ResponseJson` whenever a canonical `consentPacket` is present. Draft hydration still repopulates the convenience booleans in memory from the packet, but the durable draft JSON now keeps the consent state in one place instead of storing the same values twice. Reason: Branch 5 should finish collapsing intake consent persistence onto the canonical packet after the earlier hydration/backfill work proved those convenience fields no longer need to be stored independently.
- **`src/PTDoc.UI/Services/IntakeApiService.cs`**, **`src/PTDoc.UI/Pages/Intake/IntakeWizardPage.razor`** — Removed the duplicated draft-to-consent-packet mapping logic from the intake UI/service layer where the input shape was already `IntakeResponseDraft`. Those paths now reuse the shared `IntakeDraftPersistence.BuildCanonicalConsentPacket` helper instead of keeping a second copy of the packet-building rules in the UI project. Reason: Branch 5 should keep collapsing consent canonicalization onto one implementation so future consent-field changes do not drift between active request builders.
- **`src/PTDoc.UI/Pages/Intake/IntakeWizardPage.razor`** — Refactored the wizard-page state adapter to build a canonical `IntakeResponseDraft` first, then derive the active consent packet and convenience booleans from the shared draft persistence helpers instead of hand-maintaining a separate `IntakeWizardState -> IntakeConsentPacket` mapping path. Reason: Branch 5 should remove the last active wizard-page-specific consent canonicalization logic so both load and submit flows consume the same canonical draft representation.
- **`src/PTDoc.UI/Services/NoteWorkspaceApiService.cs`**, **`tests/PTDoc.Tests/Notes/Workspace/NoteWorkspaceApiServiceTests.cs`** — Restricted flat-list exercise/manual-technique normalization to payloads that do not already carry a canonical `StructuredPayload`. Normal v2 workspace saves now prefer the structured `exerciseRows` and `generalInterventions` collections exclusively, while explicit legacy callers without structured payloads still get compatibility normalization. Reason: Branch 5 should keep shrinking active workspace fallback behavior so legacy flat lists only influence truly legacy payloads instead of leaking into current v2 save flows.
...
### Changed - dry-needling workspace v2 migration

#### Remove the last active v1 note dependency from the workspace editor
- **`src/PTDoc.Application/Notes/Workspace/WorkspaceContracts.cs`**, **`src/PTDoc.Infrastructure/Notes/Workspace/NoteWorkspaceV2Service.cs`**, **`src/PTDoc.UI/Services/NoteWorkspaceApiService.cs`**, **`tests/PTDoc.Tests/Notes/Workspace/NoteWorkspaceApiServiceTests.cs`**, **`tests/PTDoc.Tests/Notes/Workspace/NoteWorkspaceV2ServiceTests.cs`** — Promoted dry-needling notes into the canonical workspace v2 payload, added server translation/backfill for legacy dry-needling draft JSON, and removed the active workspace editor's fallback to `/api/v1/patients/{patientId}/notes` and v1 note save endpoints. Dry-needling loads and saves now round-trip through `/api/v2/notes/workspace`, while legacy dry-needling drafts are translated into `payload.dryNeedling` and backfilled to schema version 2 on load. Reason: Branch 5 should eliminate the last active v1 note dependency in the workspace path before the remaining compatibility/dead-code cleanup.

### Changed - workspace compatibility cleanup

#### Remove dead post-migration fallback surfaces
- **`src/PTDoc.Application/Notes/Workspace/WorkspaceContracts.cs`**, **`src/PTDoc.Infrastructure/Notes/Workspace/NoteWorkspaceV2Service.cs`**, **`src/PTDoc.UI/Components/Notes/Models/ObjectiveVm.cs`**, **`src/PTDoc.UI/Components/Notes/Workspace/DailyTreatment/ObjectiveSection.razor`**, **`src/PTDoc.UI/Services/NoteWorkspaceApiService.cs`**, **`tests/PTDoc.Tests/Notes/Workspace/NoteWorkspaceApiServiceTests.cs`**, **`tests/PTDoc.Tests/Notes/Workspace/NoteWorkspaceV2ServiceTests.cs`**, **`tests/PTDoc.Tests/UI/Notes/StructuredWorkspaceEditorsTests.cs`** — Removed the now-dead `RequiresLegacyCompatibility` workspace response flag, deleted the unused daily-treatment objective compatibility component, and removed flat `TherapeuticExercises` / `ManualTechniques` fallback inputs plus their normalization path from the active workspace mapper. Structured `exerciseRows` and `generalInterventions` are now the only supported exercise/intervention authoring shape in the workspace editor. Reason: after the dry-needling v2 migration, these fallback surfaces were no longer reachable on any active path and were only preserving dead compatibility code.
- **`src/PTDoc.UI/Components/Notes/Workspace/DailyTreatment/ObjectiveSection.razor.css`** — Removed the orphaned scoped CSS file that was still paired to the deleted `ObjectiveSection.razor` component. Reason: Blazor scoped CSS requires a matching `.razor` file, so leaving the stylesheet behind caused an immediate compile failure after the component cleanup.

### Changed - progress tracking canonical note parsing
