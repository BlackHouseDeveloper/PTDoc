...
### Changed - note workspace subjective intake catalog alignment

#### Subjective note workspace now reuses intake catalogs for supplemental selections without a schema bump
- **`src/PTDoc.UI/Components/Notes/Models/SubjectiveVm.cs`**, **`src/PTDoc.UI/Services/NoteWorkspacePayloadMapper.cs`**, **`src/PTDoc.UI/Services/NoteWorkspaceApiService.cs`**, **`src/PTDoc.UI/Components/Notes/Workspace/SubjectiveTab.razor`**, **`src/PTDoc.UI/Components/Notes/Workspace/SubjectiveTab.razor.css`**, **`src/PTDoc.UI/Pages/Patient/NoteWorkspacePage.razor`**, **`tests/PTDoc.Tests/UI/Notes/StructuredWorkspaceEditorsTests.cs`**, **`tests/PTDoc.Tests/Notes/Workspace/NoteWorkspacePayloadMapperTests.cs`**, **`tests/PTDoc.Tests/Notes/Workspace/NoteWorkspaceApiServiceTests.cs`**, **`tests/PTDoc.Tests/Notes/Workspace/NoteWorkspaceV2ServiceTests.cs`**, **`tests/PTDoc.Tests/UI/Pages/NoteWorkspacePageTests.cs`** — Replaced the note workspace’s hard-coded living-situation and comorbidity lists with the same intake reference catalog service used by intake, split home layout into its own catalog-backed note UI selector while continuing to encode it through `WorkspaceSubjectiveV2.OtherLivingSituation`, added catalog-backed assistive-device and medication selection with legacy/manual text fallback, moved the workspace V2 subjective parse/compose logic into a dedicated UI payload mapper, and updated seed-chip/test coverage so review surfaces continue to show human-readable labels instead of IDs. Reason: align note subjective editing with intake’s source-of-truth catalogs in `codex/refdata-intake-alignment` without changing `WorkspaceSubjectiveV2`, bumping `WorkspaceSchemaVersions.EvalReevalProgressV2`, or adding a database migration.
...
### Fixed - Workspace subjective intake catalog compatibility and export parity

- **`src/PTDoc.Application/Notes/Workspace/WorkspaceSubjectiveCatalogNormalizer.cs`** — Added a shared workspace-subjective catalog normalizer that canonicalizes supplemental intake IDs and labels to human-readable intake labels, preserves unmatched free text, and handles the transitional house-layout parse/compose path through `OtherLivingSituation`. Reason: note UI and export surfaces needed one source of truth for legacy ID compatibility without changing the workspace schema.
- **`src/PTDoc.UI/Services/NoteWorkspacePayloadMapper.cs`**, **`src/PTDoc.UI/Pages/Patient/NoteWorkspacePage.razor`** — Rewired note workspace mapping and review chips to use the shared normalizer so assistive devices, living situation, home layout, comorbidities, and medications load old ID-backed values as canonical intake labels instead of leaking IDs or collapsing them into free text. Reason: Branch 1 required active note authoring and review surfaces to stay aligned with intake catalogs and preserve legacy/manual content.
- **`src/PTDoc.Infrastructure/Pdf/ClinicalDocumentHierarchyBuilder.cs`**, **`src/PTDoc.Api/Program.cs`** — Injected intake reference data into the PDF hierarchy builder and added human-readable subjective export summaries for assistive devices, living situation, home layout, comorbidities, and medications. Reason: exported/PDF note summaries needed to match the normalized review surfaces and avoid emitting raw catalog IDs.
- **`tests/PTDoc.Tests/Notes/Workspace/NoteWorkspacePayloadMapperTests.cs`**, **`tests/PTDoc.Tests/Notes/Workspace/NoteWorkspaceApiServiceTests.cs`**, **`tests/PTDoc.Tests/UI/Notes/StructuredWorkspaceEditorsTests.cs`**, **`tests/PTDoc.Tests/UI/Pages/NoteWorkspacePageTests.cs`**, **`tests/PTDoc.Tests/Integration/PdfIntegrationTests.cs`** — Extended regression coverage to use persisted supplemental/medication IDs and assert that UI load/save, editor rendering, review chips, and PDF export all normalize them back to the same intake labels. The PDF integration helper now normalizes extracted text for QuestPDF/PdfPig quirks, and the new export test validates the exact normalized home-layout value on the hierarchy plus stable readable label fragments in the rendered PDF text. Reason: lock the compatibility path down across mapper, page, and export regressions without making the PDF assertions brittle to renderer text extraction quirks.
