...
IDisposable.OnDisposing – not a lifecycle method per se, but if a component implements IDisposable/IAsyncDisposable, Blazor calls Dispose when the component is removed. Use this to clean up timers, event handlers, or JS objects to prevent memory leaks. **Important for PTDoc**: Components subscribing to service events (like ISyncService.OnSyncStateChanged or IConnectivityService.OnConnectivityChanged) must implement IDisposable and unsubscribe in Dispose() to prevent memory leaks. See GlobalHeader component for the standard pattern.
